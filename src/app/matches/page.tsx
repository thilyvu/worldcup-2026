...
export default async function MatchesPage({ searchParams }: { searchParams: Promise<{ round?: string }> }) {
  ...
  const [matches, preds, players] = await Promise.all([getMatches(round), getAllPredictions(), getPlayers()]);
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <div className="anim-up d1" style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
        {ROUND_ORDER.map((r) => {
          ...
          return (
            <a key={r} href={`/matches?round=${r}`} style={{
      ...
      {[...groups.entries()].map(([group, groupMatches], gi) => (
